...
// Ambil semua scenario kategori Suspension System dari database
$stmt = $pdo->prepare("SELECT * FROM scenarios WHERE category = ? ORDER BY id DESC");
$stmt->execute(['Suspension System Problems']);
...
<head>
  ...
  <title>Suspension Problems - FixSense</title>
...
<body>
  <header>
    <h2>FixSense - Suspension System Troubleshooting</h2>
  </header>
  <div class="container">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
      ...
      <div style="display: flex; gap: 10px;">
        ...
        <a href="suspension_tools.php" class="tools-button">
          <i class="fas fa-wrench"></i> Suspension Tools
        </a>
      </div>
    </div>
    <h1>Suspension System Problems</h1>
    <ul class="problem-list">
      <!-- Hardcoded problems -->
      <?php foreach ($scenarios as $s): ?>
    <li>
        <a href="simulationSuspension.php?scenario_id=<?= $s['id'] ?>">
